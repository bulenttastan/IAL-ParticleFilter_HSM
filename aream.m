function ar = aream( X,Y,x,y,varargin )
% AREAM Calculates the area between two polygons
% X,Y first polygon
% x,y second polygon
%6th optional = display
%7th optional = plot
% Example:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% clear; %close all; clc;
% load X; load Y; load path; x=path(:,1)'; y=path(:,2)'; isdisp=1; isplot=1;
...
if isempty(find(a==X(end))) || isempty(find(b==Y(end)))
    a(end+1)=X(end);
    b(end+1)=Y(end);
end
...
if isempty(a)
    ar = ar + polyarea([x fliplr(X)],[y fliplr(Y)]);
    if isdisp; disp('no cross'); end;
    if isplot; area([x fliplr(X)],[y fliplr(Y)],'FaceColor','c'); end;
end
