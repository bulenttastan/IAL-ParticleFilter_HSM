...
isplot=[0 0 0 0]; %org, fj, shc, vn
issave=[0 0 0 0];
isplotocc=1; %together occl
issaveocc=1;
lineWidth=3;
...
%     close all;
    ...
    for i=1:size(XX,1)
        ...
        warning off;
        if layout==1 && ~isempty(find(i==[2 3 5 6 8 9]));continue; end
        if layout==3 && ~isempty(find(i==[4 5 8 10]));continue; end
        disp(i);
