function [x,y,ind]=curveintersect(varargin)
% Curve Intersections.
...
while ind1<length(x1)
    ...
    while ind2<length(x2)
        ...
        elseif ind_x2(ind2(1))==0 & ind_x1(ind1(1))~=0
            ...
            s='second';
        elseif ind_x2(ind2(1))~=0 & ind_x1(ind1(1))~=0
            [x_loc,y_loc,ind_loc]=curveintersect_local(x1(ind1),y1(ind1),x2(ind2),y2(ind2));
            s='three';
            data=ind1(1);
        elseif ind_x2(ind2(1))==0 & ind_x1(ind1(1))==0
            [x_loc,y_loc]=deal([]);
        ...
        y=[y; y_loc(:)];
        if ~isempty(x_loc)
            disp([s '=> ' num2str(data)]);%ind=[ind; ind_loc(:)];
            ind=[ind; data];
        end
        ind2=ind2(end);
    end
...
% ----------------------------------------------
function [x,y,ind]=curveintersect_local(x1,y1,x2,y2)
...
[x,y,ind]=mminvinterp(xx,yy,0); % find zero crossings of difference
if ~isempty(x)
   y=interp1(x1,y1,x);
else
   x=[];
...
%--------------------------------------------------------------------------
%--------------------------------------------------------------------------
function [xo,yo,indo]=mminvinterp(x,y,yo)
